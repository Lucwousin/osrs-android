...
@RestrictTo(value={Scope.LIBRARY_GROUP}) public class ReportFragment extends Fragment {
    ...
    static ReportFragment get(Activity arg1) {
        return arg1.getFragmentManager().findFragmentByTag("androidx.lifecycle.LifecycleDispatcher.report_fragment_tag");
    }

    public static void injectIfNeededIn(Activity arg3) {
        FragmentManager v3 = arg3.getFragmentManager();
        if(v3.findFragmentByTag("androidx.lifecycle.LifecycleDispatcher.report_fragment_tag") == null) {
            v3.beginTransaction().add(new ReportFragment(), "androidx.lifecycle.LifecycleDispatcher.report_fragment_tag").commit();
