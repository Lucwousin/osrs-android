...
@RestrictTo(value={Scope.LIBRARY_GROUP}) public class ActionBarPolicy {
    ...
    public int getTabContainerHeight() {
        TypedArray v0 = this.mContext.obtainStyledAttributes(null, R$styleable.ActionBar, R$attr.actionBarStyle, 0);
        int v1 = v0.getLayoutDimension(R$styleable.ActionBar_height, 0);
        Resources v2 = this.mContext.getResources();
        if(!this.hasEmbeddedTabs()) {
            v1 = Math.min(v1, v2.getDimensionPixelSize(R$dimen.abc_action_bar_stacked_max_height));
        }
